...
class MLPClassifier(nn.Module):
    ...
    def forward(self, x, y = None):
        ...
        if y is not None:
            ...
            TP = ((pred == 1) & (y.data.view_as(pred) == 1)).cpu().sum().item()
            TN = ((pred == 0) & (y.data.view_as(pred) == 0)).cpu().sum().item()
            FN = ((pred == 0) & (y.data.view_as(pred) == 1)).cpu().sum().item()
            FP = ((pred == 1) & (y.data.view_as(pred) == 0)).cpu().sum().item()
            try:
                p = TP / float(TP + FP)
            except:
                p = 0
