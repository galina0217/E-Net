#include "/home/xujiarong/baseline/pytorch_DGCNN/lib/include/gnn_lib.h"
                      ...
                      int is_directed)
{
    ...
    prefix_sum.clear();
    unsigned edge_cnt = 0, node_cnt = 0, noise_cnt = 0;
    for (int i = 0; i < num_graphs; ++i)
    {
        node_cnt += num_nodes[i];
        edge_cnt += num_edges[i];
        noise_cnt += num_noises[i];
    ...
    int x2, y2, cur_noise = 0;
    for (int i = 0; i < num_graphs; ++i)
    {
        int *noise_pairs = static_cast<int *>(list_of_noise_pairs[i]);
        for (int j = 0; j < num_noises[i] * 2; j += 2)
        {
            x2 = prefix_sum[i] + noise_pairs[j];
            y2 = prefix_sum[i] + noise_pairs[j + 1];
            batch_graph->AddNoise(cur_noise, x2, y2);
            if (!is_directed)
                batch_graph->AddNoise(cur_noise + 1, y2, x2);
            cur_noise += 2;
        }
    }
